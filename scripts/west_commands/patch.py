...
class Patch(_ProjectCommand):
    ...
    def do_add_parser(self, parser_adder):
        ...
        parser.add_argument('-c', '--clean', action='store_true',
                            help='use "git am --abort" instead of "git am"')
    ...
    def do_run(self, args, ignored):
        ...
        am = 'apply --reject --whitespace=fix' if args.apply else 'am --whitespace=fix'
        failed = []
        for project in self._cloned_projects(args):
            ...
            for patch in patches:
                try:
                    if args.clean:
                        log.inf(f'git {am} --abort')
                        project.git(am.split() + ['--abort'] )
                    else:
