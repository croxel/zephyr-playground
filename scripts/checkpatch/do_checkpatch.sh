...
# Folders to be checked as part of the project code
dirs_to_check="app drivers include samples subsys tests"
files=$(for d in ${dirs_to_check}; do find $d/ -type f -name '*.[ch]'; done)
for i in $files; do
	${ZEPHYR_BASE}/scripts/checkpatch.pl --mailback --no-tree -f --emacs --summary-file \
	--show-types --ignore BRACES,PRINTK_WITHOUT_KERN_LEVEL,SPLIT_STRING \
	--max-line-length=100 $i >> $1
done
